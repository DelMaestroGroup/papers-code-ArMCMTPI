...
def split_by_type(pos_sel, type_sel, b_sel):
    ...
    mask_o2  = (type_sel == 4)
    mask_h   = (type_sel == 5)
    return (pos_sel[mask_si1], b_sel[mask_si1],
            pos_sel[mask_o1], b_sel[mask_o1],
            pos_sel[mask_si2], b_sel[mask_si2],
            pos_sel[mask_o2], b_sel[mask_o2],
            pos_sel[mask_h], b_sel[mask_h])
...
def main():
    ...
    pos_sel, type_sel, b_sel = select_mcm_cylinder(pos_mcm, type_mcm, x0, y0, args.R, keep_types=[1,2,3,4,5])
    pos_si1, b_si1, pos_o1, b_o1, pos_si2, b_si2, pos_o2, b_o2, pos_h, b_h = split_by_type(pos_sel, type_sel, b_sel)

    print(f"Natoms in the restricted region R={args.R}: Si(1)={pos_si1.shape[0]}, O(1)={pos_o1.shape[0]}, Si(2)={pos_si2.shape[0]}, O(2)={pos_o2.shape[0]}, H={pos_h.shape[0]}")

    #Ar-Ar only:
    I_ArAr, S_ArAr, nf1 = average_IQ_SQ_ArAr_over_frames(frames, Q)

    #Ar-MCM only: previous restriction based on the LJ params
    ...
    #Ar-MCM only: new additions
    I_ArSi1, S_ArSi1, nf5 = average_IQ_SQ_ArMCM_over_frames(frames, Q, pos_si1, b_si1)
    I_ArH, S_ArH, nf6 = average_IQ_SQ_ArMCM_over_frames(frames, Q, pos_h, b_h)

    #Restricted intensity based on the LJ params
    I_rest = I_ArAr + I_ArO1 + I_ArSi2 + I_ArO2

    #Total approximate intensity "loaded-empty":
    I_total = I_rest + I_ArSi1 + I_ArH
    #I_total is intensity averaged per frame

    #Total and restricted structure factor:
    if symmetricNorm==True:
        S_rest = S_ArAr + S_ArO1 + S_ArSi2 + S_ArO2
        S_total = S_rest + S_ArSi1 + S_ArH
    else:
        Nar_avg = np.mean([pos.shape[0] for _, pos in frames if pos.shape[0] > 0])
        norm_Ar = Nar_avg * (b_Ar*b_Ar)
        S_rest = I_rest / norm_Ar
        S_total = I_total / norm_Ar

    #Generating a single huge output:
    header=("#Q I_ArAr I_ArO1 I_ArSi2 I_ArO2  I_ArSi1  I_ArH I_rest I_tot S_ArAr S_ArO1 S_ArSi2 S_ArO2 S_ArSi1 S_ArH S_rest S_total\n")
    out = np.column_stack([Q, I_ArAr, I_ArO1, I_ArSi2, I_ArO2, I_ArSi1, I_ArH, I_rest, I_total, S_ArAr, S_ArO1, S_ArSi2, S_ArO2, S_ArSi1, S_ArH, S_rest, S_total])
    with open(args.out, "w") as f:
        f.write(header)
        np.savetxt(f, out, fmt="%.4f %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e")
