...
def calculate_rms(rtz_file):
    ...
    sigma_corr = np.sqrt(np.bincount(inverse, weights=deviations**2) / counts)
...
for seed in seeds:
    ...
    for snap in snapshots:
        ...
        all_results[seed][snap] = {"r": r,"Ubar": Ubar,"sigma": sigma}
...
for seed in seeds:
    ...
    Ubar_seed = 0.5 * (result1["Ubar"] + result2["Ubar"])
    sigma_seed = 0.5 * (result1["sigma"] + result2["sigma"])
    seed_out = f"{outdir}/RMS_seed{seed}_snapshot_average.dat"

    np.savetxt(seed_out,np.column_stack((r, Ubar_seed, sigma_seed)),
        header="r_A Umean_snapshot_avg_K sigma_corr_snapshot_avg_K",fmt="%.10f")

    seed_results[seed] = {"r": r,"Ubar": Ubar_seed,"sigma": sigma_seed}


# Averaging over the 5 independent seeds
r = seed_results[1]["r"]

Ubar_all_seeds = np.array([seed_results[seed]["Ubar"] for seed in seeds])
sigma_all_seeds = np.array([seed_results[seed]["sigma"] for seed in seeds])
...
np.savetxt(final_out,np.column_stack((r,Ubar_mean,sigma_mean,sigma_sem)),
    header=("r_A Umean sigma_corr_mean sigma_corr_SEM"),fmt="%.10f")
